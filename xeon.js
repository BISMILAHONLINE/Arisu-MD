...
XeonLft = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/d460e086f9f9bf6b04e17.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
                ...
                let xeonName = num
                const xtime = moment.tz('Asia/Singapore').format('HH:mm:ss')
	            const xdate = moment.tz('Asia/Singapore').format('DD/MM/YYYY')
	            const xmembers = metadata.participants.length
                let unicorndoc = {key: {fromMe: false,"participant":"0@s.whatsapp.net", "remoteJid": "6281276242128-1604595598@g.us"}, "message": {orderMessage: {itemCount: 9999999,status: 200, thumbnail: XeonWlcm, surface: 200, message: `${metadata.subject}`, orderTitle: 'xeon', sellerJid: '0@s.whatsapp.net'}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true}
                xeonbody = `┌─❖
│「 𝗛𝗶 👋 」
└┬❖ 「 @${xeonName.split("@")[0]}  」
   │✑  𝗪𝗲𝗹𝗰𝗼𝗺𝗲 𝘁𝗼 
...
XeonBotInc.sendMessage(anu.id, buttonMessage, {quoted:unicorndoc})
                	...
                	const xeonbuffer = await getBuffer(ppuser)
                    const xeontime = moment.tz('Asia/Singapore').format('HH:mm:ss')
	                const xeondate = moment.tz('Asia/Singapore').format('DD/MM/YYYY')
                	let xeonName = num
